...
public class HateoasTestController {
     ...
     * (Hateoas까지 구현했을 때, Restfull의 성숙도 마지막 단계인 3단계 API라고 한다.)
     * */
    ...
    public ResponseEntity<ResponseMessage> findAllUsers() {
        ...
        // EntityModel : hateoas가 제공하는 클래스, rest api를 만들때 해당 자원과 관련된 링크를 포함할 수 있게 해준다.
        List<EntityModel<UserDTO>> userWithRel
                = users.stream().map(user -> EntityModel.of(
                user,
                linkTo(methodOn(HateoasTestController.class).findUserByNumber(user.getNo())).withSelfRel(),
